...
export const OurProjects: React.FC = () => {
  ...
  if (isMounted && isMobile) {
    return (
      <section
        ref={sectionRef}
        className="w-full bg-[var(--color-secondary-500)] px-[var(--spacing-5)] py-[var(--spacing-15)] flex flex-col gap-[var(--spacing-10)]"
        data-name="OurProjectsMobile"
      >
        <div className="flex flex-col gap-[var(--spacing-4)]">
          <h2 className="h2-desktop text-inverse !text-[length:var(--text-9xl)] !leading-[length:var(--leading-super-loose)] m-0">
            Of Course, We Aren't All Talk
          </h2>
          <p className="p3-main-body-text text-inverse opacity-90 m-0 !text-[16px]">
            See the results behind our reputation.
          </p>
          <Link href="/our-works" className="btn btn-outline-white w-fit mt-[var(--spacing-2)]">
            View More
          </Link>
        </div>

        {/* Mobile projects list */}
        <div className="flex flex-col gap-[var(--spacing-10)] mt-[var(--spacing-6)]">
          {PROJECTS.map((project, i) => (
            <Link
              key={i}
              href={project.href}
              className="flex flex-col gap-[var(--spacing-4)] w-full group cursor-pointer"
            >
              <div className="relative w-full aspect-[4/3] rounded-[var(--radius-xl)] overflow-hidden bg-zinc-900/10">
                <img
                  src={project.src}
                  alt={project.title}
              ...
              <div className="flex flex-col gap-[var(--spacing-2)]">
                <h3 className="text-inverse italic text-2xl font-serif m-0">{project.title}</h3>
                <p className="p3-main-body-text text-inverse opacity-90 m-0 !text-[16px]">
                  {project.description}
                </p>
              </div>
